...
$.extend($.jgrid,{
	...
	getCellIndex : function (cell) {
		...
		if ($.support.msie) { return $.inArray(c, c.parentNode.cells); }
		return c.cellIndex;
	},
...
//			rowNum: 20,
			...
			colModel: [],
			rowNum : 10,//每页显示记录数
			rowList : [ 10, 20, 30 ],//下拉框，改变每页显示的记录条数
			colNames: [],
			sortorder: "asc",
			sortname: "",
//			datatype: "xml",
...
//			viewrecords: false,
			...
			prmNames: {
				...
				page : "pageNum",
			sort: "sidx",
			order: "sord", search:"search", nd:"nd", id:"id",oper:"oper",editoper:"edit",addoper:"add",deloper:"del", subgridid:"id", npage: null, totalrows:"totalrows"},
			forceFit:false,
			gridstate : "visible",
			cellEdit: false,
//			cellsubmit: "remote",
		...
		isMSIE = $.support.msie ? true:false,
		isSafari = $.support.webkit || $.support.safari ? true : false;
		ts = this;
		...
		if (isMSIE && $.support.version <= 6) {
			ii = '<iframe style="display:block;position:absolute;z-index:-1;filter:Alpha(Opacity=\'0\');" src="javascript:false;"></iframe>';
		} else { ii="";}
		$("<div class='ui-widget-overlay jqgrid-overlay' id='lui_"+this.id+"'></div>").append(ii).insertBefore(gv);
		$("<div class='loading ui-state-default-jq ui-state-active-jq tydic_pages_loading' id='load_"+this.id+"'><img src='"+application.basePath+"images/loading.gif'/><br/>"+this.p.loadtext+"</div>").insertBefore(gv);
		...
		addRowNum = function (pos,irow,pG,rN) {
			...
			return "<td role=\"gridcell\" class=\"ui-state-default-jq jqgrid-rownum\" "+prp+">"+v+"</td>";
		},
		...
		updatepager = function(rn, dnd) {
			...
			if(pgboxes) {
				...
				if(ts.p.pgbuttons===true) {
					if(cp<=0) {cp = last = 0;}
					if(cp==1 || cp === 0) {
						$("#first"+tspg+", #prev"+tspg).addClass('ui-state-disabled').removeClass('ui-state-hover2');
						if(ts.p.toppager) { $("#first_t"+tspg_t+", #prev_t"+tspg_t).addClass('ui-state-disabled').removeClass('ui-state-hover2'); }
					} else {
						$("#first"+tspg+", #prev"+tspg).removeClass('ui-state-disabled');
						if(ts.p.toppager) { $("#first_t"+tspg_t+", #prev_t"+tspg_t).removeClass('ui-state-disabled'); }
					}
					if(cp==last || cp === 0) {
						$("#next"+tspg+", #last"+tspg).addClass('ui-state-disabled').removeClass('ui-state-hover2');
						if(ts.p.toppager) { $("#next_t"+tspg_t+", #last_t"+tspg_t).addClass('ui-state-disabled').removeClass('ui-state-hover2'); }
					} else {
        ...
        setPager = function (pgid, tp){
			...
			if(ts.p.pgbuttons===true) {
				...
				pgl += "<td id='"+po[0]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon-jq ui-icon-seek-first'>第一页</span></td>";
				pgl += "<td id='"+po[1]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon-jq ui-icon-seek-prev'>上一页</span></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				pgl += "<td id='"+po[2]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon-jq ui-icon-seek-next'>下一页</span></td>";
				pgl += "<td id='"+po[3]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon-jq ui-icon-seek-end'>尾页</span></td>";
			...
			$('.ui-pg-selbox',"#"+pgcnt).bind('change',function() {
				...
				ts.p.search = true;
				ts.p.postData.totalRows = ts.p.records;
			...
			$(".ui-pg-button","#"+pgcnt).hover(function(e){
				if($(this).hasClass('ui-state-disabled')) {
					...
					$(this).addClass('ui-state-hover2');
					this.style.cursor='pointer';
				}
			},function(e) {
				if($(this).hasClass('ui-state-disabled')) {
				} else {
					$(this).removeClass('ui-state-hover2');
					this.style.cursor= "default";
				}
			});
		...
		for(i=0;i<this.p.colNames.length;i++){
			...
			thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name+"' role='columnheader' class='ui-state-default-jq ui-th-column ui-th-"+dir+"'"+ tooltip+">";
		...
		$("th",thr).each(function ( j ) {
			...
			if(ts.p.colModel[j].resizable){
				...
				if(!$.support.opera) { $(res).css("cursor","col-resize"); }
		...
		$(grid.hDiv)
			...
			.addClass("ui-state-default-jq ui-jqgrid-hdiv")
			.append(hb);
		$(hb).append(hTable);
		hTable = null;
		...
		$(grid.bDiv)
			.append($('<div style="position:relative;'+(isMSIE && $.support.version < 8 ? "height:0.01%;" : "")+'"></div>').append('<div></div>').append(this))
...
$.jgrid.extend({
	...
	showHideCol : function(colname,show) {
		return this.each(function() {
			var $t = this, fndh=false, brd=$.support.webkit||$.support.safari? 0: $t.p.cellLayout, cw;
			if (!$t.grid ) {return;}
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			if (!this.grid ) {return;}
			var $t = this, cw,
			initwidth = 0, brd=$.support.webkit||$.support.safari? 0: $t.p.cellLayout, lvc, vc=0, hs=false, scw=$t.p.scrollOffset, aw, gw=0,
			cl = 0,cr;
		...
		return this.each(function(){
			...
			function postIt() {
				...
				if(ret[0] && !rp_ge[$t.p.id].processing) {
					...
					//var aaa=$.isFunction(rp_ge[$t.p.id].serializeEditData) ? rp_ge[$t.p.id].serializeEditData(postdata) :  postdata;
	...
	navGrid : function (elem, o, pEdit,pAdd,pDel,pSearch, pView) {
		...
		return this.each(function() {
			...
			for(var i = 0; i<clone; i++) {
				...
				sep = "<td class='ui-pg-button ui-state-disabled' style='width:4px;'></td>",
				pgid, elemids;
				if(i===0) {
					pgid = elem;
					elemids = $t.p.id;
				...
				$('td',ind).each( function(i) {
					...
					if ( nm != 'cb' && nm != 'subgrid' && nm != 'rn') {
						...
						if(cm[i].editable===true) {
							...
							//Again IE
							if(cm[i].edittype == "select" && cm[i].editoptions.multiple===true && $.support.msie) {
								$(elc).width($(elc).width());
							}
							cnt++;
						}
...
setSubGrid : function () {
	return this.each(function (){
		...
		$t.p.colModel.unshift({name:'subgrid',width: $.support.safari ?  $t.p.subGridWidth+$t.p.cellLayout : $t.p.subGridWidth,sortable: false,resizable:false,hidedlg:true,search:false,fixed:true});
		cm = $t.p.subGridModel;
		if(cm[0]) {
