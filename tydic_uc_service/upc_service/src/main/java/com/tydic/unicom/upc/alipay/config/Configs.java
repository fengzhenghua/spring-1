package com.tydic.unicom.upc.alipay.config;
...
public class Configs {
    ...
    private static String signType ;//= "RSA2";     // 签名类型  
    
    private static String decodSingTypeInstance; // 支付宝异步通知 签名方式
    ...
    private static long heartbeatDelay = 5; // 交易保障线程第一次调度延迟（秒）

    private static long heartbeatDuration = 900; // 交易保障线程调度间隔（秒）
	...
	@PostConstruct
	public void init(){
		openApiDomain = upcPropertiesVo.getOpenApiDomain();
		mcloudApiDomain = upcPropertiesVo.getMcloudApiDomain();
		signType = upcPropertiesVo.getSignType();
		decodSingTypeInstance = upcPropertiesVo.getSignature();
	}
	...
	public static String getDecodSingTypeInstance() {
		return decodSingTypeInstance;
	}
}
