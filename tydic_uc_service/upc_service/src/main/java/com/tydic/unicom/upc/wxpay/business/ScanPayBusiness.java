package com.tydic.unicom.upc.wxpay.business;

import com.tydic.unicom.upc.wxpay.common.Configure;
import com.tydic.unicom.upc.wxpay.common.Log;
import com.tydic.unicom.upc.wxpay.common.Signature;
import com.tydic.unicom.upc.wxpay.common.Util;
import com.tydic.unicom.upc.wxpay.common.report.ReporterFactory;
import com.tydic.unicom.upc.wxpay.common.report.protocol.ReportReqData;
import com.tydic.unicom.upc.wxpay.common.report.service.ReportService;
...
public class ScanPayBusiness {
	...
	private static Logger logger = Logger.getLogger(ScanPayBusiness.class);
    ...
    //每次调用撤销API的等待时间
    private int waitingTimeBeforeReverseServiceInvoked = 3000;
    ...
    public void run(ScanPayReqData scanPayReqData, ResultListener resultListener) throws Exception {
    	...
    	String key = scanPayReqData.getKey();
        ...
        ReportReqData reportReqData = new ReportReqData(
        		...
        		scanPayReqData.getSub_mch_id(),
        		scanPayReqData.getKey(),
        ...
        if (Configure.isUseThreadToDoReport()) {
            ...
            logger.info("pay+report总耗时（异步方式上报）：" + (timeAfterReport - costTimeStart) + "ms");
        } else {
            ReportService.request(reportReqData);
            timeAfterReport = System.currentTimeMillis();
            logger.info("pay+report总耗时（同步方式上报）：" + (timeAfterReport - costTimeStart) + "ms");
        }
        ...
        if (scanPayResData.getReturn_code().equals("FAIL")) {
            ...
            logger.error("【支付失败】支付API系统返回失败，请检测Post给API的数据是否规范合法");
            ...
            //--------------------------------------------------------------------
            if (!Signature.checkIsSignValidFromResponseString(payServiceResponseString, scanPayReqData.getKey())) {
            ...
            if (scanPayResData.getResult_code().equals("SUCCESS")) {
                ...
                if (errorCode.equals("AUTHCODEEXPIRE") || errorCode.equals("NOTENOUGH") || errorCode.equals("AUTH_CODE_INVALID")) {
                	...
                	ScanPayQueryResData resData = doPayQueryLoop(appid, mchid, key, payQueryLoopInvokedCount, outTradeNo);
                	if (resData != null) {
                        logger.info("【需要用户输入密码、查询到支付成功】");
                        scanPayResData.setReturn_code(resData.getReturn_code());
                        scanPayResData.setResult_code(resData.getResult_code());
                        scanPayResData.setTransaction_id(resData.getTransaction_id());
                        resultListener.onSuccess(scanPayResData);
                    } else {
     ...
     * @throws Exception
     */
    private ScanPayQueryResData doOnePayQuery(String appid,String mchid, String key, String outTradeNo) throws Exception {

        sleep(waitingTimeBeforePayQueryServiceInvoked);//等待一定时间再进行查询，避免状态还没来得及被更新

        String payQueryServiceResponseString;

        ScanPayQueryReqData scanPayQueryReqData = new ScanPayQueryReqData(appid, mchid, key, "",outTradeNo);
     ...
     */
    private ScanPayQueryResData doPayQueryLoop(String appid,String mchid, String key,int loopCount, String outTradeNo) throws Exception {
        //至少查询一次
        if (loopCount == 0) {
            loopCount = 1;
        }
        ...
        for (int i = 0; i < loopCount; i++) {
        	ScanPayQueryResData resData = doOnePayQuery(appid, mchid, key, outTradeNo);
            if (resData != null) {
                return resData;
            }
        }
     ...
     * @throws Exception
     */
    private boolean doOneReverse(String appid,String mchid, String key, String outTradeNo) throws Exception {

        sleep(waitingTimeBeforeReverseServiceInvoked);//等待一定时间再进行查询，避免状态还没来得及被更新

        String reverseResponseString;

        ReverseReqData reverseReqData = new ReverseReqData(appid, mchid, key, "",outTradeNo);
        reverseResponseString = reverseService.request(reverseReqData);

        logger.info("撤销API返回的数据如下：");
        logger.info(reverseResponseString);
        if(reverseResponseString == null || reverseResponseString.equals("")){
        ...
        if (reverseResData == null) {
            logger.info("支付订单撤销请求逻辑错误，请仔细检测传过去的每一个参数是否合法");
            return false;
        }
        if (reverseResData.getReturn_code().equals("FAIL")) {
            //注意：一般这里返回FAIL是出现系统级参数错误，请检测Post给API的数据是否规范合法
            logger.info("支付订单撤销API系统返回失败，失败信息为：" + reverseResData.getReturn_msg());
            return false;
        } else {
            if (reverseResData.getResult_code().equals("FAIL")) {
                logger.info("撤销出错，错误码：" + reverseResData.getErr_code() + "     错误信息：" + reverseResData.getErr_code_des());
     ...
     * @throws InterruptedException
     */
    private void doReverseLoop(String appid,String mchid, String key, String outTradeNo) throws Exception {
        //初始化这个标记
        needRecallReverse = true;
        //进行循环撤销，直到撤销成功，或是API返回recall字段为"Y"
        while (needRecallReverse) {
            if (doOneReverse(appid, mchid, key, outTradeNo)) {
                return;
            }
        }
    }
