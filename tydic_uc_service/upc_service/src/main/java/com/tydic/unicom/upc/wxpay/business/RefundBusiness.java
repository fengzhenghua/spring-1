...
public class RefundBusiness {
	...
	private static Logger logger = Logger.getLogger(RefundBusiness.class);
    public RefundBusiness(String certPath, String certPassword) throws IllegalAccessException, ClassNotFoundException, InstantiationException, UnrecoverableKeyException, KeyManagementException, SecurityException, NoSuchMethodException, IllegalArgumentException, NoSuchAlgorithmException, KeyStoreException, InvocationTargetException, IOException {
        refundService = new RefundService(certPath, certPassword);
    }
    ...
    public void run(RefundReqData refundReqData,ResultListener resultListener) throws Exception {
        ...
        ReportReqData reportReqData = new ReportReqData(
        		refundReqData.getAppid(),
        		refundReqData.getMch_id(),
        		refundReqData.getSub_mch_id(),
        		refundReqData.getKey(),
        ...
        if (refundResData.getReturn_code().equals("FAIL")) {
            ...
            if (!Signature.checkIsSignValidFromResponseString(refundServiceResponseString, refundReqData.getKey())) {
