...
import com.tydic.unicom.upc.wxpay.protocol.refund_query_protocol.RefundOrderData;
...
public class RefundQueryBusiness {
	...
	private static Logger logger = Logger.getLogger(RefundQueryBusiness.class);
    ...
    public void run(RefundQueryReqData refundQueryReqData,RefundQueryBusiness.ResultListener resultListener) throws Exception {
        ...
        ReportReqData reportReqData = new ReportReqData(
        		refundQueryReqData.getAppid(),
        		refundQueryReqData.getMch_id(),
        		refundQueryReqData.getSub_mch_id(),
        		refundQueryReqData.getKey(),
        ...
        if (refundQueryResData.getReturn_code().equals("FAIL")) {
            ...
            if (!Signature.checkIsSignValidFromResponseString(refundQueryServiceResponseString, refundQueryReqData.getKey())) {
    ...
    private void getRefundOrderListResult(String refundQueryResponseString) throws ParserConfigurationException, SAXException, IOException {
        ...
        for(RefundOrderData refundOrderData : refundOrderList){
        	logger.info("退款订单数据NO" + count + ":");
        	logger.info(refundOrderData.toMap());
