...
@Service("InfoServiceOrderExtServDu")
public class InfoServiceOrderExtServDuImpl implements InfoServiceOrderExtServDu{
	...
	@Override
	public List<InfoServiceOrderExtVo> queryInfoServiceOrderExtByOrderNo(
			InfoServiceOrderExtVo vo) throws Exception {
		InfoServiceOrderExtPo InfoServiceOrderExt = new InfoServiceOrderExtPo();
		BeanUtils.copyProperties(vo, InfoServiceOrderExt);
		List<InfoServiceOrderExtVo> listVo = new ArrayList<InfoServiceOrderExtVo>();
		List<InfoServiceOrderExtPo> listPo = infoServiceOrderExtServ.queryInfoServiceOrderExtByOrderNo(InfoServiceOrderExt);
		if(listPo != null && listPo.size()>0){
			for(InfoServiceOrderExtPo po : listPo){
				InfoServiceOrderExtVo ordVo = new InfoServiceOrderExtVo();
	...
	public boolean updateInfoServiceOrderExtFileSix(InfoServiceOrderExtVo vo) throws Exception {
		...
		List<InfoServiceOrderExtPo> listPo = infoServiceOrderExtServ.queryInfoServiceOrderExtByOrderNo(InfoServiceOrderExt);
		if(listPo != null && listPo.size()>0){
			return infoServiceOrderExtServ.updateInfoServiceOrderExt(InfoServiceOrderExt);
		}else{
			return infoServiceOrderExtServ.insertInfoServiceOrderExt(InfoServiceOrderExt);
		}
	}
	
	@SuppressWarnings("unchecked")
	@Override
	public Map<String, Object> getAttrDesc(InfoServiceOrderExtVo vo,String tableName)
			throws Exception {
