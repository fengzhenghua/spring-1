...
import com.tydic.unicom.uoc.business.common.interfaces.ActivemqProcessFlowListenerServDu;
...
public class ActivemqProcessFlowListenerServDuImpl implements ActivemqProcessFlowListenerServDu{

	Logger logger = Logger.getLogger(ActivemqProcessFlowListenerServDuImpl.class);
	...
	public void getMessageFromProcessFlow(Object message) throws Exception {
		...
		logger.info("=================消息队列（ProcessFlow）监听=====================");
		...
		String className = serviceMap.get(activemqSendPo.getService_code()+"Class").trim();
		String methodName = serviceMap.get(activemqSendPo.getService_code()+"Method").trim();
		...
		try {
			...
			logger.error("===============ActivemqProcessFlowListenerServDuImpl异常，转人工任务==============");
			//e.printStackTrace();
			ProcInstTaskInstVo procInstTaskInstVo = new ProcInstTaskInstVo();
			BeanUtils.copyProperties(activemqSendPo.getTaskInst(), procInstTaskInstVo);
			changeToArtificialServiceServDu.changeToArtificialService(procInstTaskInstVo, "102");		}
	...
	public Map<String,String> getValueFromProperties(String key) throws Exception{
		...
		InputStream in = ActivemqProcessFlowListenerServDuImpl.class.getResourceAsStream("/serviceCode.properties");
		props.load(in);
