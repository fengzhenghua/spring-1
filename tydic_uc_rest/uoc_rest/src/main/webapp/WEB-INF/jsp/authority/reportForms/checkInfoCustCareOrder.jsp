...
<script type="text/javascript">
		...
		audit.addr_change_submit=function(opType){
			...
			var cust_care_id=$("#cust_care_id").val();
			var cust_name=$("#cust_name").val();
			var id_number=$("#id_number").val();
			var self_date=$("#self_date").val();
			var cust_addr=$("#cust_addr").val();
			var ofr_code=$("#ofr_code").val();
			...
			var addr_desc=$("#addr_desc").val();
			var addr_memo=$("#addr_memo").val();
			var addr_id=$("#addr_id").val();
			var kxmemo=$("#kxmemo").val();
			var memo=$("#memo").val();
			var area_id=$("#select_area").val();
			var kx_oper_id='<%=load_oper_no%>';

		
			var orderState="2";
			if(opType=="N"){
				var orderState="-2";
				if(kxmemo==null||kxmemo==""){
					alert("请在备注栏写上是否选择有线通资源！");
					return false;
				}else{
					 var r=confirm("确认已经在备注栏上写上是否选择有线通资源？")
					  if(r==true){
			...
			var dataParam={"oper_type":"2","jsessionid":"","save_flag":"","order_state":orderState,"cust_care_id":cust_care_id,"cust_name":cust_name,"id_number":id_number,"id_type":"1","self_date":self_date,"cust_addr":cust_addr,"ofr_code":ofr_code,"ofr_name":ofr_name,"link_name":link_name,"link_phone":link_phone,"oper_id":oper_id,"oper_name":oper_name,"oper_phone":oper_phone,"addr_desc":addr_desc,"addr_memo":addr_memo,"addr_id":addr_id,"kxmemo":kxmemo,"memo":memo,"area_id":area_id,"kx_oper_id":kx_oper_id};
			$.ajax({  
                loading: '正在加载中...',  
                type: "POST",
	            dataType: "json",
                url: application.fullPath + "csrest/custRsp/saveCustRspOrder",  
                data: dataParam,  
	...
	<div class="search-input">
          <table  border="0" align="center" cellpadding="0" cellspacing="0">
            ...
            <tr>
              ...
              <td  style="display:none" align="center"><input type="text" name="self_date" id="self_date"  class="input_click"  readonly="readonly"/></td>
            </tr>
              <tr>
            ...
            <tr>
              ...
              <td  align="center"><select  ltype="init" name="select_area" id="select_area" url="<%=fullPath%>authority/webUtilUssM165/queryRuleAreaHx" class="sel"></td>
            </tr>
            <tr>
