...
<%
    ...
    if("nx".equals(province))
    	code =memQueryServ.queryCodeType("code_type_rpt_business_list_serv_code_nx");
...
<head>
    ...
    <script type="text/javascript">
        var audit = {
            rep_list_array: ["operating_receipt", "income_list", "business_list", "terminal_list", "mobile_number_list", "sampling_audit_list","business_statistics_list"],
            rep_list_manager: {
        ...
        audit.initClick = function () {
            ...
            $("#select_store_number_delete").click(function () {
                audit.empty.emptyInput("select_store_number");
            });
            $("#select_uniform_oper_delete").click(function () {
                audit.empty.emptyInput("select_uniform_oper");
            });
            $("#tele_type_all").click(audit.tele_type_all);
            $("input[name='tele_type']").click(function(){
            	$("#tele_type_all").attr("checked", false);
            });
        ...
        audit.commmonCss = function () {
            ...
            $("#select_audit_status_tr").hide();
            $("#serv_code_box").hide();
            $("#select_store_number_tr").hide();
            $("#select_uniform_oper_tr").hide();
            audit.empty.emptyInput("select_number");
        ...
        audit.operatingReceiptCss = function () {
            this.commmonCss();
            if(province =="nx"){
           	 $("#select_store_number_tr").show();
             $("#select_uniform_oper_tr").show();
//              $("#serv_code_box").show();
           }
        };
...
//              $("#serv_code_box").show();
        		 ...
        		 audit.rep_list_manager[value] =
                     $("#rep_" + value).ligerGrid({
                         columns: [
                             ...
                             {display: '门店编码', name: 'store_number', width: 150},
                             {display: '统一工号', name: 'create_uniform_oper', width: 150}
                         ],
                         delayLoad: true,
            	...
            	audit.rep_list_manager[value] =
                    $("#rep_" + value).ligerGrid({
                        columns: [
                            ...
                            {display: '营业厅名称', name: 'dept_no', width: 300},
							{display: '可选活动', name: 'activity_name', width: 300},
							{display: '渠道编码', name: 'chnl_code', width: 100},
							{ display: '电子协议查询', isAllowHide:'true', name: 'order_id', id:'order_id', width: 300 ,render: function(rowdata, rowindex, value){	
							    var rucan="";
								if(rowdata.order_id){
								 rucan="<a style='text-decoration:underline' href=\"javascript:showpdf("+"'"+rowdata.order_id+"'"+",'"+1+"'"+");\">开户协议</a>&nbsp"+    //增加参数判断协议类型 1为开户协议2为靓号协议3为补充协议
        ...
        audit.samplingAuditList = function (value) {
            audit.rep_list_manager[value] =
                    $("#rep_" + value).ligerGrid({
                        ...
                        //营业厅名称 接待人员 受理时间 是否已稽核 稽核是否通过 处理意见
                        columns: [
							...
							{ display: '电子协议查询', name: 'id_seq' ,render: function(rowdata, rowindex, value){
                                var rucan = "<a href=\"javascript:showAuditPic2(" + "'" + rowdata.id_seq + "','" + rowdata.order_id + "','" + rowdata.customer_name + "','" + rowdata.any_dog1 + "','" + rowdata.audit_flag + "','" + rowdata.audit_status + "','" + rowdata.audit_note + "'" + ");\">稽核</a>";
                                //alert(rucan);
                                return rucan;
							},width: 200}/*,
        ...
        audit.setParams = function () {
            ...
            params.store_number =$("#select_store_number").val();
            params.create_uniform_oper =$("#select_uniform_oper").val();
        ...
        audit.rpt_export = function () {
            ...
            if (audit_report_type == "operating_receipt") {
                var colNames = "tele_type,stat_date,serv_code,charge_code,pay_type,pay_charge,derate_charge,discount_charge,oper_local_net,oper_dept_no,oper_no,store_number,create_uniform_oper";
                post(application.fullPath + "authority/rptBusiFee/QueryRptBusiFeeSumExcel", colNames);
            }
            else if (audit_report_type == "income_list") {
                var colNames = "tele_type,device_number,serv_code,charge_code,pay_type,pay_charge,derate_charge,discount_charge,oper_local_net,oper_dept_no,oper_no,oper_date,store_number,create_uniform_oper";
                ...
                var colNames = "tele_type,serv_code_name,device_number,customer_name,product_name,oper_no,oper_date,dept_no,chnl_code,order_id,store_number,create_uniform_oper";
                ...
                var colNames = "order_id,acc_nbr,user_no,tele_type,order_sub_type,customer_id,customer_name,user_statu_code,product_name,product_id,id_type,id_number,operator_id,operator_name,login_oper_no,login_oper_name,id_seq,audit_status,audit_flag,any_dog1,audit_note,dept_no,create_date,local_net";
...
// 						$("#show_audit4").attr("href",pdfUrl4);
        ...
        //获取身份证图片
        function showAuditPic2(id_seq, order_id, customer_name, any_dog1, audit_flag, audit_status, audit_note) {
            var m_id_seq = id_seq;
                ...
                success: function (message) {
                    ...
                    if (any_dog1 == "eq") {
                        if (audit_flag == "已稽核"/*&&audit_status=="通过")*/) {
                            //全部按钮 都不能点 只能看
                            $("#audit_pass").attr("href", "javascript:audit_show_warning(1);");
                            $("#audit_no_pass").attr("href", "javascript:audit_show_warning(1);");
                            $("#audit_again").attr("href", "javascript:audit_show_warning(1);");
                            $("#audit_note").show();
                        } else {
                            //可以点通过或者不通过
        ...
        <div class="table_box">
            <table width="446" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                    ...
                    <td width="124">全选</td>
                    <td width="24" align="center"><input type="checkbox" value="2G" name="tele_type" ></td>
                    <td width="123">2G</td>
                    <td width="26" align="center"><input type="checkbox" value="3G" name="tele_type" ></td>
                    <td width="125">3G</td>
                </tr>
                <tr>
                    <td height="22" align="center"><input type="checkbox" value="4G" name="tele_type" ></td>
                    <td> 4G</td>
                    <td align="center"><input type="checkbox" value="M165" name="tele_type" ></td>
                    <td> 宽带</td>
                    <td align="center">&nbsp;</td>
                    <td>&nbsp;</td>
                </tr>
                <tr id="wo_lan" style="display:none;">
                	<td align="center"><input type="checkbox" value="LAN" name="tele_type" ></td>
                    <td> 智慧沃家</td>
                    <td align="center"><input type="checkbox" value="WO_FA" name="tele_type" ></td>
                    <td> 沃家庭</td>
                </tr>
            </table>
        </div>
    ...
    <div class="box box_margin_right" id="select_rang_box">
        ...
        <div class="table_box_none">
            <table width="486" border="3" align="center" cellpadding="3" cellspacing="3">
                ...
                <tr id="select_store_number_tr">
                    <td width="114" height="22" align="right"><strong>门店编码：</strong></td>
				  <td width="372" align="center"><div id="select_store_number_delete"  class="input_click_delete"></div>
                        <a href="javascript:void(0);">
					<input type="text" name="select_store_number" id="select_store_number"  class="input_click" >
                        </a>
                    </td>
                </tr>
                <tr id="select_uniform_oper_tr">
                    <td width="114" height="22" align="right"><strong>统一工号：</strong></td>
				  <td width="372" align="center"><div id="select_uniform_oper_delete"  class="input_click_delete"></div>
                        <a href="javascript:void(0);">
					<input type="text" name="select_uniform_oper" id="select_uniform_oper"  class="input_click" >
                        </a>
                    </td>
                </tr>
    ...
    <table style="margin:0 auto">
        <tr>
            <td><a class="audit_leble" href="javascript:get_pic_pre();"><</a></td>
					<td><Iframe height='226' width='836' src="" id="identitypic" scrolling="auto" ></iframe></td>
					<td><Iframe height='226' width='836' src="" id="getPdf"  scrolling="auto" style="display:none;"></iframe></td>
    ...
    <div id="audit_table">
        <div id="audit_note_div">
            <div class="select">
                ...
                <div class="btn">
							<a id="show_audit"  href="" target="_blank"  onClick="javascript:chg_color(this);">协议展示</a>
                </div>
			<div class="btn">
