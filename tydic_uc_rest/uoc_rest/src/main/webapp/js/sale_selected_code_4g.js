$(document).ready(function() {
	  ...
	  var limit_money=$.trim($("#limit_money").val());			//套餐月最低消费
	  ...
	  function selectPage() {
		  ...
		  $("#sale_selected_code").html('');//先清空套餐显示标签
		   $("#layerPaging").pages({
			   ...
			   getData:function(){
				   return {"tele_type":tele_type,"pay_flag":pay_flag,"market_price":limit_money};
			   },
		  ...
		  if(activity_type==""){
		   ...
		   $.ajax({
				...
				success: function(data){
					...
					if(jsonObj.type=='success'){
						var activity_type = $("#activity_type").val()
						if(!(activity_type=='ZSYW001') && !(activity_type=='CFSF001')){
							$("#terminal_style_name").show();
							$("#terminal_style").show();
						}
...
//	   });
	   ...
	   $("#guarantee_type").change(function(){
		  ...
		  if(activity_type=='ZSYW001' || activity_type=='CFSF001'){
			   $.ajax({
					type: "POST",
					url: "getActivityNameNoTerminal",
					data:data,//需要传的数据
					dataType: "json",
					success: function(data){
						var jsonObj=eval(data);
						if(jsonObj.type=='success'){
							$("#terminal_brand_td").hide();
							$("#terminal_brand_name_td").hide();
							$("#terminal_model_td").hide();
							$("#terminal_model_name_td").hide();
							$("#activity_name_lbl").show();
							$("#activity_name").show();
							$("#activity_name").html(jsonObj.args.options);//向activity_type添加option
						}else{
		...
		if(activity_type=='YCMP001'||activity_type=='GJRM001'||activity_type=='ZBJM001'||activity_type=='HYSJ001'||activity_type=='HYHJ001'){

			   $.ajax({
					type: "POST",
					url: "getTerminalBrand",
					data:data,//需要传的数据
					dataType: "json",
					success: function(data){
						var jsonObj=eval(data);
						if(jsonObj.type=='success'){
							$("#terminal_brand_td").show();
							$("#terminal_brand_name_td").show();
							$("#terminal_model_td").show();
							$("#terminal_model_name_td").show();
							$("#terminal_brand").show();
							$("#terminal_brand_name").show();
							$("#terminal_brand_name").html(jsonObj.args.options);//向activity_type添加option
						}else{
							$.message({
									type:'error', //可以是3个，success.error.warn
									content:jsonObj.content//错误信息
	   ...
	   /*--------------------------------------------------选择品牌---------------------------------------*/
	   $("#terminal_brand_name").change(function(){
		   var terminal_brand_code=$("#terminal_brand_name").val();
		   var data={
				   "terminal_brand_code":terminal_brand_code
		   }
		   HideOfSelectTerminalBrand();//选择，请选则时，需要隐藏的标签
		   /*初始化数据*/
		  ClearOfCheckTerminal();		//清空终端校验信息
		  ClearOfActivityDesc();		//清空活动描述信息
		  ClearOfActivitySelectedItem();//清空活动选中信息
		  ClearOfFormActivityInfo();	//清空表单中活动信息
		  if(terminal_brand_code==''){
			  return;
		  }
		   $.ajax({
				type: "POST",
				url: "getTerminalModel",
				data:data,//需要传的数据
				dataType: "json",
				success: function(data){
					var jsonObj=eval(data);
					if(jsonObj.type=='success'){
						$("#terminal_model").show();
						$("#terminal_model_name").show();
						$("#terminal_model_name").html(jsonObj.args.options);//向activity_type添加option
					}else{
						$.message({
								type:'error', //可以是3个，success.error.warn
								content:jsonObj.content//错误信息
	   ...
	   /*--------------------------------------------------选择型号---------------------------------------*/
	   $("#terminal_model_name").change(function(){
		   var activity_type=$("#activity_type").val();//活动类型
		   var guarantee_type=$("#guarantee_type").val();
		   var terminal_model_code=$("#terminal_model_name").val();
		   var data={
				   "ofr_id":$("#ofr_id_result").val(),//套餐ID
				   "ofr_type":$("#ofr_type_result").val(),
				   "ofr_sub_type":$("#ofr_sub_type_result").val(),//套餐类型
				   "guarantee_type":guarantee_type,//担保类型
				   "tele_type":tele_type,//电信类型
				   "activity_type":activity_type,//活动类型
				   "terminal_model_code":terminal_model_code//型号
		   }
		   HideOfSelectTerminalModel();//选择，请选则时，需要隐藏的标签
		   /*初始化数据*/
		  ClearOfCheckTerminal();		//清空终端校验信息
		  ClearOfActivityDesc();		//清空活动描述信息
		  ClearOfActivitySelectedItem();//清空活动选中信息
		  ClearOfFormActivityInfo();	//清空表单中活动信息
		  if(terminal_model_code==''){
			  return;
		  }
		   $.ajax({
				type: "POST",
		   ...
		   $.ajax({
				...
				success: function(data){
					...
					if(jsonObj.type=='success'){
						...
						$("#activity_type_name_val").val(jsonObj.args.activity_info.activity_type_name);		//活动类型名称
						$("#activity_guarantee_type_val").val(jsonObj.args.activity_info.activity_sub_type);//担保类型
						$("#activity_guarantee_name_val").val(jsonObj.args.activity_info.activity_sub_type_name);//担保名称
						...
						$("#activity_type_name_result").val($("#activity_type_name_val").val());							//活动ID
						$("#activity_guarantee_type_name_result").val($("#activity_guarantee_name_val").val());							//活动ID
	   ...
	   $("#btn_submit").click("click",function(){
		   ...
		   var terminal_style = $("#terminal_style").find("option:selected").val();
		  // var terminal_style_code=$("#terminal_style").val();
		   $("#terminal_style_result").val(terminal_style);
		   
		   var terminal_model_code=$("#terminal_model_name").val();
		   var terminal_brand_name=$("#terminal_brand_name").val();
		   $("#terminal_model_result").val(terminal_model_code);
		   $("#terminal_brand_result").val(terminal_brand_name);
		   
		   var terminal_id=$.trim($("#terminal_id").val());
		   $("#terminal_id_result").val(terminal_id);
		   /*判断是否选中套餐*/
		   if(ofr_id==""){
			   $.alert("请选择套餐");
			   return;
		   ...
		   if(!terminal_id&&!terminal_info_flag){
			   $.alert("终端号不能为空");
			   return;
		   }
	  ...
	  function HideOfSelectPackage(){
		  ...
		  $("#terminal_brand").hide();
		  //隐藏  品牌
		  $("#terminal_brand_name").hide();
		  //隐藏 型号标签
		  $("#terminal_model").hide();
		  //隐藏 型号
		  $("#terminal_model_name").hide();
		  //隐藏可选活动 标签
	   ...
	   * 选择品牌时，需要隐藏的标签
	   */
	  function HideOfSelectTerminalBrand(){
		  //隐藏 型号标签
		  $("#terminal_model").hide();
		  //隐藏 型号
		  $("#terminal_model_name").hide();
	   ...
	   * 选择型号时，需要隐藏的标签
	   */
	  function HideOfSelectTerminalModel(){
		  //隐藏可选活动 标签
		  $("#activity_name_lbl").hide();
		  //隐藏可选活动
		  $("#activity_name").hide();
