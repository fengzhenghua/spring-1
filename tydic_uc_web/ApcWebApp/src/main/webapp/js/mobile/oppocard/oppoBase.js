...
$(document).ready(function () {
	...
	selfServFlag=oper_code_apc=CACHE_UTIL.getSessionItem("selfServFlag");
	...
	//选发展人
	$("#developer").unbind("click").bind("click",function(){
		getDeveloperInfo();
	});
	//发展人分页显示，左翻页
	$("#dev_btn_ahead").unbind("click").bind("click", turnDevLeft);
	//发展人分页显示，右翻页
	$("#dev_btn_behind").unbind("click").bind("click", turnDevRight);
	//关闭发展人选择框
	$("#hidedev").unbind("click").bind("click", function(){
		DIALOG_UTIL.hideDialog("developer_pop");
	});
...
function getDeveloperInfo(){
	initDevPage();
	devPageData();
	DIALOG_UTIL.showDialog("developer_pop", true);
	/*return;
	var dataJson={
			"jsessionId":oper_no,
			"developerNo":"",
...
function initDevPage() {
	dev_totalRows = dev_list.length;
	dev_page_total = parseInt(dev_totalRows/dev_pageNums)+(parseInt(dev_totalRows%dev_pageNums)>0?1:0);
	$("#dev_page_total").html(dev_page_total);
 ...
 * 发展人分页显示
 */
function devPageData() {
	$("#dev_number_content").html("");
	var startRow = (dev_page_current-1)*dev_pageNums;
	var endRow = startRow+dev_pageNums;
	var result = '';
	for (var i=startRow;i<endRow&&i<dev_totalRows;i++) {
		var start ='';
		var end ='';
		if((i+1)%3==1){
			start='<div class="number_box">';
		...
		result += start + '<div class="number_box_row" style="cursor:pointer;" id=\'' + dev_list[i].developer_no + '\'>' + dev_list[i].developer_name + '</div>' + end;
	}
	$("#dev_number_content").html(result);

	$("#dev_page_current").html(dev_page_current);
 ...
 * 发展人左翻页
 */
function turnDevLeft() {
	if (dev_page_current<=1) {
		return;
	}
	dev_page_current--;
	devPageData();
 ...
 * 发展人右翻页
 */
function turnDevRight() {
	if (dev_page_current>=dev_page_total) {
		return;
	}
	dev_page_current++;
	devPageData();
 ...
 * 点击一个发展人，进行选择
 */
function dev_selectClick() {
	$("#dev_number_content .number_box_row").unbind("click").bind("click", function (e) {
		dev_id=$(this).attr("id");
		dev_name=$(this).html();
		$("#selected_developer").html(dev_name);
		select_dev_id=dev_id;
		select_dev_name=dev_name;
