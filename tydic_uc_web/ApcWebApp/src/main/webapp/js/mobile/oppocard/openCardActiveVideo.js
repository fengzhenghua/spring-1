...
$(document).ready(function(){
	...
	function updateOrderAttr(){
		...
		var dataJson={
				"order_id":order_id,
				"video_file_name":livingExamineInfo.args.video_file_name,
				"video_live_sim":livingExamineInfo.args.video_live_sim,
				"order_status":livingExamineInfo.args.order_status,
				"video_server_id":video_server_id,
				"compare_fail_times":livingExamineInfo.args.compare_fail_times
		};
		$.ajax({
			type: "post",
			...
			success: function(data) {
				if (data.respCode=="0000") {
					if (livingExamineInfo.respCode=="0000") {
						updateSaleOrder();
					} else {
						var fail_times = livingExamineInfo.args.compare_fail_times;
						//活体比对，连续比对三次失败的需要提示客户转人工激活
						if(fail_times >= 3){
							$("#pop_confirm .tip").text("激活失败：活动认证校验连续三次失败，请转人工激活方式进行卡激活。")
							updateSaleOrder();
						}else{
							DIALOG_UTIL.showTypeDialog("error", livingExamineInfo.content);
							DIALOG_UTIL.hideDialog("", "loading");
						}
					}
				} else {
